...
⚠️ 断词铁律（edge-tts 无法按 SSML 控制，只能改文案）：
  ...
  - 想让某个词组连读/纠正多音字，唯一的可靠办法是改写成自然说法：
    ...
    · 整体术语（"机载端"、"据此"）直接写进句子即可连读，**不要额外加逗号标记**
      ——逗号会造成停顿感，反而不连贯；若确实要断句（如"编码端据此动态调整
      冗余度"要断成"编码端 / 据此 / 动态调整"），用**顿号（、）**而不用逗号：
      "编码端、据此、动态调整冗余度"，顿号停顿短、读起来更顺
...
SEGS = [
    ...
    ("seg5_demo", "下面展示实测效果。机载端回传的视频经编码和公网传输，地面站解码播放稳定流畅。"),  # 实测（机载端 直接连读，不加逗号）
    # 结束页如需配音可加
]
# ==========================================================
