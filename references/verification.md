...
- **标点规范复核**：`subs.srt` 每行行尾不得出现 。！？；，、：· 与空格（`clean_punct` 已剥，若手改过文案需复查）；**全片不得出现分号；**，行中只允许偶尔的逗号/顿号/冒号——句末标点(。！？)与分号应在 split_lines 的硬断行处拆成了独立字幕（`make_subs.py` 的 main 里有硬校验，出现即报错）。

## 8. 交付前铁律自检

- [ ] 所有卡片图片均为用户提供的 PPT 原图（`pptx_extract/ppt/media/`），无额外图片。
- [ ] 所有卡片内容最下缘 ≤ y940，字幕烧录后无任何文字/图片被遮挡（`verify_video.py --cards` diff<4）。
- [ ] 字幕行尾无标点、全片无分号；术语（前向纠错/弱网场景/机载端等）未被断行；字幕与配音同源对得上。
- [ ] **混音响度一致**：逐段 `ffmpeg -ss <段起点> -t 2.5 -i 成品.mp4 -af volumedetect`，各段 mean_volume 应落在同一小段区间（如 -21~-24dB），不许出现逐段递增的响度阶梯（demo 段不得突然比前段响）。
